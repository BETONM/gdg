...
import { CheckinPayload } from '@/types/mission';
...
export function DailyCheckIn({ onComplete }: DailyCheckInProps) {
  ...
  const toggleListening = () => {
    if (!recognition) {
      alert('현재 사용 중인 브라우저에서는 음성 인식을 지원하지 않습니다. (Chrome, Safari 등 권장)');
      return;
    }
  ...
  const handleSubmit = () => {
    ...
    onComplete({ mode: 'voice', transcript: freeText });
  };
      ...
      <div className="bg-gradient-to-r from-[var(--spot-green)] via-[var(--spot-blue)] to-[var(--spot-yellow)] px-5 pt-safe pb-4 sticky top-0 z-10">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className="w-10 h-10 rounded-xl bg-white overflow-hidden shadow-sm flex-shrink-0 flex items-center justify-center border border-white/20">
              <img src="/logo.png" alt="Hi Spot Daily" className="w-full h-full object-contain scale-[1.35]" />
            </div>
            <div>
              <h1 className="text-lg font-bold text-white drop-shadow-md">Hi Spot Daily</h1>
              <p className="text-xs text-white/90 mt-0.5 font-medium">{today}</p>
            </div>
          </div>
      ...
      <div className="px-4 py-6 space-y-5 pb-24">
        ...
        <div className="space-y-3 pt-2">
          <button
            ...
            className="w-full py-4 px-5 bg-gradient-to-r from-[var(--spot-green)] to-[var(--spot-blue)] text-white rounded-xl text-base font-bold hover:shadow-xl transition-all shadow-lg flex items-center justify-center gap-2 disabled:opacity-50"
          >
